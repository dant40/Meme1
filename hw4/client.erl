...
do_join(State, Ref, ChatName) ->
	Channels = State#cl_st.con_ch,
	case maps:find(ChatName,Channels) of
		{ok, Value} -> State#cl_st.gui!{result,self(),Ref,err};
   ...
   %% {{dummy_thicc, dummy_response}, State}.

%% executes `/leave` protocol from client perspective
do_leave(State, Ref, ChatName) ->
	Channels = State#cl_st.con_ch,
	case maps:find(ChatName,Channels) of
		{ok, Value} -> whereis(server)!{self(), Ref,leave,ChatName},
					   receive
						   {PID, Ref, ackleave} ->
