...
do_register(State, Ref, ClientPID, ClientNick) ->
	NewReg = maps:put(ClientNick,ClientPID,State#chat_st.registrations),
	NewState = #chat_st  {name = State#chat_st.name, registrations = NewReg, history = State#chat_st.history},
...
do_unregister(State, ClientPID) ->
	NewRegs = maps:remove(ClientPID,State#chat_st.registrations),
	NewState = #chat_st  {name = State#chat_st.name, registrations = NewRegs, history = State#chat_st.history}.
	%loop(NewState).
...
do_update_nick(State, ClientPID, NewNick) ->
	NewRegs = maps:update(ClientPID,NewNick,State#chat_st.registrations),
	NewState = #chat_st  {name = State#chat_st.name, registrations = NewRegs, history = State#chat_st.history}.
...
do_propegate_message(State, Ref, ClientPID, Message) ->
	ClientPID!{self(), Ref, ackmsg},
	CliNick = maps:get(ClientPID,State#chat_st.registrations),
	OtherPIDS = lists:delete(ClientPID,maps:keys(State#chat_st.registrations)),
	lists:map(fun(PID) -> PID!{request, self(), Ref,{incomingmsg, CliNick,State#chat_st.name, Message}} end, OtherPIDS),
	NewHist = [Message|State#chat_st.history],
	NewState = #chat_st {name = State#chat_st.name, registrations = State#chat_st.registrations, history = NewHist}.
